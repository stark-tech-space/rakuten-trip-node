...
import { AmenityCode, CategoryId } from './types';
...
export type GetRoomsV2Input = {
  propertyCode: string;
};

export type BedRoom = {
  beds: Array<Record<string, number>>;
  name: string;
  isPrivate: boolean;
};

export type LivingRoom = any;
...
export type GetRoomsV2Output = {
  propertyCode: string;
  roomTypeCode: number;
  isRoomOpened?: boolean | null;
  roomCategory?: string | null;
  title?: Partial<Record<Lang, string>> | null;
  minAdultOccupancy: number;
  maxAdultOccupancy: number;
  minChildOccupancy: number;
  maxChildOccupancy: number;
  isSharing?: boolean | null;
  roomGender?: string | null;
  maxRollaways?: number | null;
  maxCribs?: number | null;
  quantity?: number | null;
  viewCode?: number | null;
  nonSmoking?: boolean | null;
  size?: string | null;
  floor?: string | null;
  bedRooms?: Array<BedRoom> | null;
  livingRooms?: Array<LivingRoom> | null;
  bathRooms?: Array<BathRoom> | null;
  equipments?: Array<Equipment> | null;
  amenities?: Array<Amenity> | null;
  images?: Array<RoomImage> | null;
  brief?: string | null;
...
export default class RakutenTrip {
  ...
  private async request(request: Request, category: ApiCategory): Promise<any> {
    const { url, method, parameters = {}, requestBody, requestHeaders = {} } = request;
  ...
  private bookingRecordTransformation(bookingRecord: BookingRecordResponse) {
    const {
      ...
      cancellation_policy: { remarks, cancellation_policies: cancellationPolicies },
      cancellation_details,
      contact_person: {
  ...
  async getMyPropertyList(input: GetMyPropertyListInput): Promise<GetMyPropertyListOutput> {
    ...
    return {
      ...
      myPropertyList: my_property_list.map(
        (property: {
          ...
          const { property_code, is_active, activated_at, deactivated_at } = property;
          return {
  ...
  async postProperties(input: PostPropertiesInput): Promise<PostPropertiesOutput> {
    const {
    ...
    Object.keys(response).forEach((hotelId) => {
      ...
      const heroImages: ImageItem[] = (hero_images || []).map(imageMappingFunction);
      ...
      output[hotelId] = {
        ...
        rooms: rooms || [],
        facilities,
        isPropertyOpened: is_property_opened,
      };
    });
  ...
  async getRoomsV2(input: GetRoomsV2Input): Promise<GetRoomsV2Output> {
    const { propertyCode } = input;
    return await this.request(
      {
        url: `/rooms_v2/${propertyCode}`,
        method: 'GET',
      },
      ApiCategory.CONTENT,
    );
  ...
  async postBookingPolicy(input: PostBookingPolicyInput): Promise<PostBookingPolicyOutput> {
    const {
      search: {
    ...
    const output: PostBookingPolicyOutput = {
      ...
      cancellationPolicy: {
        ...
        cancellationPolicies: cancellation_policy.cancellation_policies.map(
          (policy: { penalty_percentage: string; date_from: string; date_to: string }) => ({
  ...
  async getBookingPolicy(input: GetBookingPolicyInput): Promise<GetBookingPolicyOutput> {
  ...
  async postPaymentGuarantee(
    ...
    const {
      bookingId,
      cardInfo: { cardNo: card_no, cardType: card_type, name, expMM: exp_mm, expYY: exp_yy, cvc },
      sessionId,
    } = input;
